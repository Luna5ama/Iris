...
TOOLS = [
    {
        "name": "reload_shader",
        "description": "Reload the active Iris shader pack in the running Minecraft client.",
...
def load_control(path: Path) -> dict:
    if not path.exists():
        raise RuntimeError(f"Iris control file does not exist: {path}")
...
def request(control_file: Path, endpoint: str, payload: dict | None = None) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Iris request failed: HTTP {exc.code}: {body}") from exc
...
def main() -> int:
    ...
    parser.add_argument(
        "--control-file",
        type=Path,
        default=Path("fabric/run/iris-capture-control.json"),
    )
            ...
            if method == "initialize":
                result(
                    message_id,
                    {
                        ...
                        "capabilities": {"tools": {}},
                        "serverInfo": {"name": "iris-capture", "version": "0.1.0"},
                    },
                )
