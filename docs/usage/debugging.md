...
Since the internal representation omits formatting, comments and whitespace, the code that is printed back out is not necessarily the same as the code that was originally provided. Iris preserves source locations through include expansion, preprocessing and transformation, then inserts `#line` directives so OpenGL compiler diagnostics refer back to the original shader-pack file and line. Iris also appends a source map to compiler errors because OpenGL drivers report source files as implementation-formatted numeric identifiers. Non-essential whitespace is omitted from normal output for better performance; enabling debug mode prints the patched code with indentation.
